...
export class CompanystaffsService {
    ...
    findAll(): Promise<CompanystaffEntity[]> {
        return this.companystaffsRepository.find({relations:['corporationEntity']});
    }

    findAllCompanystaffs(createCompanystaff: CreateCompanystaffDTO) : Promise<CompanystaffEntity[]> {
        let query = this.companystaffsRepository.createQueryBuilder("companystaff");
        query.leftJoinAndSelect("companystaff.corporationEntity", "corporationEntity");
        ...
        if(typeof createCompanystaff.staff_id !== 'undefined') {
            query.andWhere("staff_id = :staff_id", { staff_id: createCompanystaff.staff_id });
        }
        if(typeof createCompanystaff.staff_name !== 'undefined') {
            query.andWhere("staff_name LIKE :staff_name", { staff_name: `%${createCompanystaff.staff_name}%` });
        }
        if(typeof createCompanystaff.job_position !== 'undefined') {
            query.andWhere("job_position = :job_position", {job_position: createCompanystaff.job_position});
        }
        if(typeof createCompanystaff.profile_source_type !== 'undefined') {
            query.andWhere("profile_source_type = :profile_source_type", {profile_source_type: createCompanystaff.profile_source_type});
        }


        if(typeof createCompanystaff.employee_from_number !== 'undefined' && typeof createCompanystaff.employee_to_number !== 'undefined') {
            query.andWhere("corporationEntity.employee_number Between :employee_from_number AND :employee_to_number", {employee_from_number: createCompanystaff.employee_from_number, employee_to_number: createCompanystaff.employee_to_number});
        }
        if(typeof createCompanystaff.corporation_name !== 'undefined') {
            query.andWhere("corporationEntity.corporation_name LIKE :corporation_name ", {corporation_name: `%${createCompanystaff.corporation_name}%`});
        }
    ...
    async update(companystaff: CreateCompanystaffDTO) {
        ...
        try{
           ...
           return await this.findAllCompanystaffs(companystaff);
            ...
            console.log('there is no staff_id and corporation_id : ' + companystaff.staff_id + " and " + companystaff.corporation_id );
            throw e;
        }
    }
